...
public interface VaccinationManagerRepository extends JpaRepository<Vaccination, Integer> {
    ...
    @Query(value = "select e.* from Vaccination e where e.vaccination_id = ?1 and e.delete_flag = ?2 and e.vaccination_type_id = ?3",
            nativeQuery = true)
    ...
    @Modifying
    @Query(value = "insert into vaccination(`date`, delete_flag, description, start_time, end_time, status, location_id, vaccination_type_id, vaccine_id) " +
    ...
    @Modifying
    @Query(value = "update vaccination set " +
     ...
     */
    @Modifying
    @Query(value = "update vaccination set delete_flag = ?1 where vaccination_id = ?2", nativeQuery = true)
     ...
     */
    @Modifying
    @Query(value = "update vaccination set status = ?1 where vaccination_id = ?2", nativeQuery = true)
    void updateVaccinationById(boolean status, int id);

    /**
     * TrungTQ code: Tìm kiếm cùng với phân trang ;
     */
    @Query(value = "select * from vaccine_management.vaccination " +
